...
/**
 * Konfigurasi terpusat AI Provider OpenRouter (Primary + Fallback)
 */
export const AI_CONFIG = {
  primaryModel: 'google/gemini-2.5-flash',
  fallbackModel: 'deepseek/deepseek-chat-v3.1',
  maxTokens: 4096,
...
function getFallbackApiKey(): string {
  const key =
    ...
    import.meta.env.OPENROUTER_API_KEY_FALLBACK ||
    getPrimaryApiKey();
  return key.trim();
}
...
function normalizeOdds(oddsInput: any): MatchOdds {
  ...
  if (typeof oddsInput === 'object') {
    if (oddsInput.home) odds.home = String(oddsInput.home);
    if (oddsInput.away) odds.away = String(oddsInput.away);
    if (oddsInput.draw) odds.draw = String(oddsInput.draw);
    if (oddsInput.over) odds.over = String(oddsInput.over);
    if (oddsInput.under) odds.under = String(oddsInput.under);
    if (oddsInput.handicap) odds.handicap = String(oddsInput.handicap);
    return odds;
  }

  if (typeof oddsInput === 'string' && oddsInput.trim() !== '' && oddsInput.toLowerCase() !== 'null') {
    odds.home = oddsInput.trim();
  }
 ...
 *
 * Flow:
 * UPLOAD IMAGE -> VALIDATE IMAGE -> CONVERT TO BASE64 -> SEND MULTIMODAL TO GEMINI 2.5 FLASH -> EXTRACT MATCH DATA
 */
export async function parseMatchScreenshotWithAi(
  ...
  try {
    ...
    if (srvResp.ok) {
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          throw new Error('Format parsing JSON screenshot tidak valid.');
        }
      }
      ...
      const schedule: DetectedSchedule = {
        sport: parsedJson?.sport && parsedJson.sport !== 'null' ? String(parsedJson.sport).trim() : 'Sepak Bola',
    ...
    if (srvErr instanceof AiServiceError) {
      throw srvErr;
    }
    console.warn('[AI Vision] Server /api/parse-screenshot error, attempting direct client fallback:', srvErr?.message);
  }
...
Return valid JSON in this exact format without any markdown backticks:
{
  ...
  try {
    response = await fetch(AI_CONFIG.endpoint, {
      ...
      headers: {
        ...
        'HTTP-Referer': typeof window !== 'undefined' ? window.location.origin : 'https://max-ai-sports.applet',
        'X-Title': 'MAX AI ANALIS PARLAY',
      },
    ...
    debugInfo.openRouterErrorMessage = netErr.name === 'AbortError'
      ? `Timeout: Gemini Vision tidak merespons dalam ${AI_CONFIG.timeoutMs / 1000} detik.`
      : netErr.message || 'Koneksi jaringan ke OpenRouter terputus.';
    throw new AiServiceError(
      `Gagal terhubung ke OpenRouter Gemini Vision: ${debugInfo.openRouterErrorMessage}`,
      AI_CONFIG.primaryModel,
      debugInfo,
      netErr
    );
  ...
  if (!response.ok) {
    ...
    if (response.status === 402) {
      throw new AiServiceError(
        `Kredit OpenRouter tidak mencukupi untuk token yang diminta (${errorDetail}).`,
        AI_CONFIG.primaryModel,
        debugInfo
      );
    }
    ...
    throw new AiServiceError(
      `OpenRouter API Error [${response.status}]: ${errorDetail}`,
      AI_CONFIG.primaryModel,
      debugInfo
    );
  }
  ...
  try {
    ...
    debugInfo.openRouterErrorMessage = 'Respons dari OpenRouter bukan format JSON yang valid.';
    throw new AiServiceError(
      'Format data dari OpenRouter tidak valid.',
      AI_CONFIG.primaryModel,
      debugInfo,
      jsonErr
    );
...
function parseMultiMatchAiOutput(
  ...
  for (let i = 0; i < inputMatches.length; i++) {
    ...
    const moneylinePick =
      aiItem?.marketAnalysis?.moneyline?.pick && aiItem.marketAnalysis.moneyline.pick !== 'null'
        ? String(aiItem.marketAnalysis.moneyline.pick)
        : origMatch.homeTeam || 'N/A';
    const moneylineConf =
      aiItem?.marketAnalysis?.moneyline?.confidence && aiItem.marketAnalysis.moneyline.confidence !== 'null'
    ...
    const handicapPick =
      aiItem?.marketAnalysis?.handicap?.pick && aiItem.marketAnalysis.handicap.pick !== 'null'
        ? String(aiItem.marketAnalysis.handicap.pick)
        : 'N/A';
    const handicapConf =
      aiItem?.marketAnalysis?.handicap?.confidence && aiItem.marketAnalysis.handicap.confidence !== 'null'
        ? String(aiItem.marketAnalysis.handicap.confidence)
        : 'N/A';

    const totalPick =
      aiItem?.marketAnalysis?.total?.pick && aiItem.marketAnalysis.total.pick !== 'null'
        ? String(aiItem.marketAnalysis.total.pick)
        : 'N/A';
    const totalConf =
      aiItem?.marketAnalysis?.total?.confidence && aiItem.marketAnalysis.total.confidence !== 'null'
    ...
    // 1. Ekstrak Data Riset (Maintenance 4)
    const research: MatchResearchData = {
      form: {
        home: aiItem?.research?.form?.home || aiItem?.form?.home || 'Data tidak ditemukan setelah pencarian.',
        away: aiItem?.research?.form?.away || aiItem?.form?.away || 'Data tidak ditemukan setelah pencarian.',
      },
      headToHead:
        aiItem?.research?.headToHead ||
        (aiItem?.headToHead ? String(aiItem.headToHead) : 'Rekor pertemuan tidak ditemukan setelah pencarian.'),
      injuries: {
        home:
          aiItem?.research?.injuries?.home ||
          (aiItem?.injuries?.home ? String(aiItem.injuries.home) : 'Tidak ada laporan cedera signifikan.'),
        away:
          aiItem?.research?.injuries?.away ||
          (aiItem?.injuries?.away ? String(aiItem.injuries.away) : 'Tidak ada laporan cedera signifikan.'),
      },
      homeAway: {
        homeRecord: aiItem?.research?.homeAway?.homeRecord || 'N/A',
        awayRecord: aiItem?.research?.homeAway?.awayRecord || 'N/A',
      },
      statistics: aiItem?.research?.statistics || (aiItem?.statistics ? String(aiItem.statistics) : undefined),
      news: aiItem?.research?.news || (aiItem?.news ? String(aiItem.news) : undefined),
    };
    ...
    if (Array.isArray(aiItem?.markets) && aiItem.markets.length > 0) {
      markets = aiItem.markets.map((m: any) => ({
        market: String(m.market || 'Market'),
        pick: String(m.pick || 'N/A'),
        confidence: String(m.confidence || '65%'),
    ...
    const bestMarket: BestSupportedMarket = {
      ...
      pick: aiItem?.bestMarket?.pick || aiItem?.suggestedPick || moneylinePick,
      reason:
    ...
    const singleResult: SingleMatchAnalysis = {
      ...
      form: {
        home: research.form?.home || 'Data tidak ditemukan setelah pencarian.',
        away: research.form?.away || 'Data tidak ditemukan setelah pencarian.',
      },
      research,
      markets,
      bestMarket,
...
async function callOpenRouter(
  ...
  if (!apiKey) {
    throw new Error('API Key OpenRouter tidak ditemukan pada environment variables.');
  }

  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), AI_CONFIG.timeoutMs);

  const isVisionCapable = model.includes('gemini') || model.includes('vision');
  
  let messageContent: any;
  if (base64DataUrl && isVisionCapable) {
    messageContent = [
      {
        type: 'text',
        text: prompt,
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 402) {
        throw new Error(`Kredit OpenRouter tidak mencukupi untuk token yang diminta (HTTP 402: ${errorDetail}).`);
      } else if (response.status === 429) {
        throw new Error(`Rate limit terlampaui (${errorDetail}).`);
      } else if (response.status === 401 || response.status === 403) {
        throw new Error(`Otentikasi OpenRouter gagal (${errorDetail}). Periksa API Key.`);
      } else {
        throw new Error(`OpenRouter merespons error [${response.status}]: ${errorDetail}`);
      }
    }
    ...
    if (!content || typeof content !== 'string' || content.trim() === '') {
      throw new Error('OpenRouter tidak mengembalikan konten teks analisis yang valid.');
    }
...
export async function analyzeMultiMatchesWithAi(
  ...
  try {
    ...
    const parsed = parseMultiMatchAiOutput(
      ...
      AI_CONFIG.primaryModel,
      usedWebSearch
        ? 'OpenRouter (Gemini 2.5 Flash + Web Research)'
        : 'OpenRouter (Gemini 2.5 Flash - Direct)'
    );
  ...
  // 2. Coba Fallback Model: deepseek/deepseek-chat-v3.1 HANYA jika Gemini benar-benar gagal API level
  let fallbackErrorMsg = '';
  try {
    if (!fallbackApiKey) {
      throw new Error('VITE_OPENROUTER_FALLBACK_KEY belum dikonfigurasi di environment variable.');
    }

    debugInfo.provider = 'OpenRouter (DeepSeek V3.1 - Fallback)';
    debugInfo.model = AI_CONFIG.fallbackModel;
    debugInfo.requestStatus = 'fallback';

    // Untuk DeepSeek (text-focused), coba dengan web search jika didukung, atau direct
    let rawResult = '';
    let httpStatus = 200;
    try {
      const resp = await callOpenRouter(
    ...
    const parsed = parseMultiMatchAiOutput(
      ...
      matches,
      AI_CONFIG.fallbackModel,
      'OpenRouter (DeepSeek V3.1 - Fallback)'
    );
  ...
  // 3. Jika Keduanya Gagal: WAJIB Error + Retry, JANGAN gunakan dummy response.
  throw new AiServiceError(
