...
export const AnalyzeView: React.FC<AnalyzeViewProps> = ({ onNavigate, onOpenAuth }) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (detected.matches.length > 0) {
        ...
        setVisionSuccess(
          `Berhasil mengenali ${detected.matches.length} pertandingan dari screenshot. Silakan periksa atau sesuaikan detail di bawah sebelum analisis.`
        );
      } else {
        // Rule 5: VALID GEMINI RESPONSE tetapi pertandingan tidak ditemukan -> JANGAN FALLBACK KE DEEPSEEK!
        setVisionWarning('Pertandingan tidak berhasil dikenali dari gambar.');
      }
    } catch (err: any) {
      ...
      // Rule 5: Jika valid respons tetapi tidak ditemukan
      if (err.message === 'Pertandingan tidak berhasil dikenali dari gambar.') {
        setVisionWarning('Pertandingan tidak berhasil dikenali dari gambar. Silakan periksa kualitas gambar atau masukkan nama tim secara manual.');
      } else {
        // API level error
        setAiError(err.message || 'Foto tidak dapat diproses. Silakan pilih gambar lain.');
      }
    } finally {
  ...
  const handleExecuteAnalysis = async () => {
    ...
    try {
      ...
      setAiError('Analisis gagal. Silakan coba lagi.');
    } finally {
      clearInterval(researchInterval);
      setAnalyzing(false);
    }
                ...
                return (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-3.5">
                      ...
                      <p className="text-xs font-medium text-slate-800">{item.research?.form?.home || item.form?.home || 'Data tidak tersedia'}</p>
                    </div>
                    <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-3.5">
                      <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 block mb-1">
                        Form Away ({item.awayTeam})
                      </span>
                      <p className="text-xs font-medium text-slate-800">{item.research?.form?.away || item.form?.away || 'Data tidak tersedia'}</p>
                    </div>
                  </div>
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-white p-4 space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                      <div className="rounded-xl bg-slate-50/80 p-3 border border-slate-200">
                        ...
                        <p className="text-slate-800 leading-relaxed">{item.research?.headToHead || 'Head to head dianalisis dari rekor kompetisi resmi.'}</p>
                      </div>

                      <div className="rounded-xl bg-slate-50/80 p-3 border border-slate-200">
                        <span className="text-[10px] font-bold uppercase text-slate-500 block mb-1">Laporan Cedera & Lineup</span>
                        <p className="text-slate-800 leading-relaxed">
                          {item.research?.injuries?.home || item.research?.injuries?.away
                            ? `Home: ${item.research?.injuries?.home || 'Lengkap'} | Away: ${item.research?.injuries?.away || 'Lengkap'}`
                            : 'Tidak ada laporan cedera pemain kunci yang signifikan.'}
                        </p>
                      </div>
                    </div>
