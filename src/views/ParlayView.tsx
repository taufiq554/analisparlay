...
export const ParlayView: React.FC<ParlayViewProps> = ({
  ...
  // Sync legs to localStorage whenever updated
  useEffect(() => {
    try {
      localStorage.setItem('max_ai_parlay_legs', JSON.stringify(legs));
    } catch {}
  }, [legs]);
  ...
  return (
    <div id="parlay-view" className="space-y-6 animate-in fade-in pb-20 max-w-7xl mx-auto">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 sm:p-8 space-y-5 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <button
                id="btn-check-correlation-pro"
                type="button"
                onClick={() => onNavigate('pro-tools')}
                className="inline-flex items-center space-x-1.5 rounded-full border border-indigo-200 bg-indigo-50 px-3.5 py-1.5 text-xs font-bold text-indigo-700 hover:bg-indigo-100 transition shadow-xs"
              >
                <ShieldAlert className="h-3.5 w-3.5 text-indigo-600" />
                <span>Cek Risiko di Alat Pro</span>
              </button>
              ...
              <button
                ...
                className="inline-flex items-center space-x-1.5 rounded-full border border-blue-200 bg-blue-50 px-3.5 py-1.5 text-xs font-bold text-blue-700 hover:bg-blue-100 transition shadow-xs"
              >
                <Share2 className="h-3.5 w-3.5 text-blue-600" />
                <span>Slip Iklan (9:16 & 1:1)</span>
              </button>
              ...
              <button
                ...
                className="inline-flex items-center space-x-2 rounded-full bg-slate-900 px-4 py-1.5 text-xs font-bold text-white hover:bg-slate-800 transition shadow-sm active:scale-95 disabled:opacity-50"
              >
                <Download className="h-3.5 w-3.5 text-blue-400" />
                <span>{isExporting ? 'Mencetak Slip...' : 'Download Slip (.PNG)'}</span>
              </button>
            </div>
          </div>
      ...
      {/* Social Share Slip Modal */}
      <SocialShareModal
        isOpen={shareModalOpen}
        onClose={() => setShareModalOpen(false)}
        title="AI PARLAY SLIP OF THE DAY"
        sport={legs[0]?.sport || 'Sepak Bola'}
        league={legs[0]?.league || 'Top European Leagues'}
        legs={legs.map((l) => ({
          match: l.match || 'Pertandingan',
          pick: l.pick || 'Rekomendasi',
          market: l.market || 'Pasar',
          odds: l.odds || '1.90',
          confidence: l.confidence || '75%',
        }))}
        totalOdds={combinedOdds || '3.85'}
        confidenceScore="79%"
      />
    </div>
  );
